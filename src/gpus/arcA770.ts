...
export const arcA770: Gpu = {
  ...
  docs: {
    die: {
      ...
      programming: 'Programmed through oneAPI: SYCL kernels via Level Zero, or OpenCL/Vulkan/DX12. No fixed ISA version scheme like sm_xx — code JITs from SPIR-V per driver.',
    },
    front: {
    ...
    xve: {
      ...
      programming: 'A sub-group of 8 maps to one XVE issue; FP64 is absent on Alchemist — doubles emulate slowly or fail at JIT.',
    },
    xmx: {
    ...
    rtu: {
      ...
      type: 'Fixed function',
      role: 'One per Xe-core: fixed-function BVH traversal and triangle intersection, plus a thread-sorting unit that regroups divergent ray-hit shading for SIMD coherence — Intel\'s answer to ray divergence ahead of NVIDIA\'s SER.',
      specs: [
        { label: 'Count', value: '32' },
    ...
    l2: {
      ...
      type: 'Cache',
      role: 'Die-wide 16 MB L2 behind all slices, connected to clients over Intel\'s memory fabric (GTI). Large for its generation — same year NVIDIA shipped 6 MB on GA102.',
      specs: [
    ...
    pcie: {
      ...
      type: 'Interconnect',
      role: 'x16 Gen4 host link. Arc performance notably depends on Resizable BAR — the driver assumes full-aperture host access to VRAM.',
      specs: [
