...
export const h100: Gpu = {
  ...
  docs: {
    die: {
      ...
      type: "Die overview",
      role: "GH100 measures 814 mm² on TSMC 4N and carries 80 billion transistors. The SXM5 product ships with 132 of 144 SMs active and 5 of 6 HBM3 stacks populated, and the positions of the disabled units vary from die to die. Die area is dominated by the eight GPC fields, a 50 MB L2 in two central partitions, and the HBM interfaces on both long edges. The floorplan is schematic and proportions are approximate.",
      specs: [
      ...
      pipeline: "Work arrives over PCIe Gen5 or NVLink, is queued by the GigaThread Engine, and is distributed as thread blocks to SMs inside the 8 GPCs. Load and store instructions from warps flow through the SM L1 to the L2 partition, then out to the HBM3 controllers on the die edges.",
      programming: "Code is compiled with -arch=sm_90 (CUDA 12+). Hopper adds three features aimed at transformer training. Thread block clusters let a group of blocks schedule together on one GPC so they can read and write each other's shared memory directly, a capability called distributed shared memory. Without it, data shared between blocks would have to access global memory. The Tensor Memory Accelerator is a dedicated copy engine in each SM that moves tensor tiles between global memory and shared memory while the warps keep computing, so the next tile arrives before the current one is consumed. FP8 stores each weight or activation in eight bits instead of sixteen. The Transformer Engine decides per layer whether the reduced precision is acceptable, which halves memory use and doubles Tensor Core throughput when it is.",
    },
    gpc: {
      ...
      pipeline: "The GigaThread Engine load-balances thread blocks across the GPCs. Within a GPC, a work distribution unit assigns blocks to individual SMs.",
      programming: "CUDA 12 thread block clusters (cluster_dims) must fit within one GPC. This constraint is what enables distributed shared memory and cluster-wide barriers between the participating SMs.",
    },
    tpc: {
    ...
    sm: {
      ...
      pipeline: "The SM receives thread blocks from GPC work distribution and schedules warps onto the partition pipelines. Loads and stores pass through the load/store units to the L1, then to the L2 and HBM. Tensor Core operands stream from registers and shared memory.",
      programming: "One thread block executes on exactly one SM, and warps of 32 threads are the scheduling unit. Declaring a variable __shared__ places it in the L1 carveout, where every thread in the block can reach it at on-chip latency instead of HBM latency. sm_90 adds the Tensor Memory Accelerator, a copy engine in each SM that moves whole tiles from global memory into shared memory through cp.async.bulk without stalling the warps, and distributed shared memory, which extends __shared__ access to the other SMs in the same thread block cluster.",
    },
    l2: {
      ...
      pipeline: "Every SM L1 miss lands here, and all HBM traffic stages through it. Atomic operations shared between SMs also resolve in the L2.",
    ...
    hbm: {
      ...
      programming: "Global memory allocated with cudaMalloc resides here. When the 32 threads of a warp read addresses that fall in the same cache line, the hardware satisfies the whole warp with one memory transaction, a pattern called coalescing. Scattered addresses each cost their own transaction and waste most of the bandwidth they occupy.",
    },
    nvlink: {
      name: "NVLink 4",
      type: "Interconnect",
      ...
      programming: "Exposed via cudaDeviceEnablePeerAccess, NCCL and NVSHMEM. NVSHMEM is a library that presents the combined HBM of every GPU in the job as one flat address space, so a kernel can load or store remote memory without an explicit communication call.",
    },
    pcie: {
      ...
      pipeline: "The entry point for host-originated work. Kernel launches and DMA descriptors pass through here to the GigaThread Engine.",
    },
    front: {
      ...
      programming: "CUDA streams and grid launches are mediated here. Whether concurrent kernels and asynchronous copies actually overlap depends on its scheduling.",
    },
    media: {
      name: "Media engines",
      type: "Fixed function",
